...
- [x] **Supabase Auth → Resend SMTP wired** -- swapped Supabase's built-in email service (capped at ~4 emails/hour project-wide, which broke real signups during App Review demo account setup) for custom SMTP via the existing verified `boges2birds.com` Resend domain. Dedicated `supabase-auth` Resend API key (sending-only, scoped to `boges2birds.com`); SMTP host `smtp.resend.com:465`, username `resend`, sender `noreply@boges2birds.com`. Site URL set to `https://boges2birds.com/email-confirmed.html` so confirmation links land on a real page instead of `localhost:3000`. Verified end-to-end with a fresh throwaway signup.
- [x] **App Review demo account + 1.0.0 submission** -- `remleynick44+appreviewer@gmail.com` provisioned via in-app signup, email confirmed, onboarding walked to completion so reviewer sign-in lands directly in the Home tab; credentials in App Store Connect → App Review Information. Build 11 (`ec55f39c-1202-494a-9be7-356b11cd0865`) attached to version `1.0.0` with store metadata, regulated medical device + content rights attestations completed, submitted to App Review on 2026-05-30.

### Next Steps (in order)

1. [ ] **Apple review + initial rollout** -- build 11 submitted to App Review on 2026-05-30 (version `1.0.0`). Watch App Store Connect for review status; Apple typically responds in 24-48 hours. On approval, release manually or phased, then monitor Sentry / RevenueCat / Supabase for the first day's crashes, subscription events, and sync errors. If rejected, address the feedback and resubmit.
