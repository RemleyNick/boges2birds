...
- [x] First production EAS build -- build 5 (commit `0d90de6`, IPA `kmNBok4RkZEfrcdEuHRGWp.ipa`) finished cleanly on the `production` profile. Native crash reporting (Sentry SDK) is compiled into the binary and works. **JS source map upload is NOT yet wired** -- see the next-steps item below; deferred per the original "fix in build 2 if needed" caveat.

### Next Steps (in order)

1. [ ] **TestFlight + sandbox subscription test** -- install build 5 (`kmNBok4RkZEfrcdEuHRGWp.ipa`) on a physical device via TestFlight, run onboarding, trigger the paywall, complete a sandbox purchase, confirm the transaction shows up in the RevenueCat dashboard. Also intentionally trigger a JS error (e.g. throw from a debug-only button) and confirm Sentry captures the event -- the stack trace will be minified until item 2 lands.
2. [ ] **Wire Sentry JS source-map upload for EAGER_BUNDLE builds** -- EAS Build 18+ bundles JS eagerly *before* xcodebuild, which short-circuits the Sentry plugin's wrapped `react-native-xcode.sh` / `sentry-xcode.sh` build phase (see `node_modules/@sentry/react-native/plugin/build/withSentryIOS.js`). `metro.config.js` already uses `getSentryExpoConfig` (commit `0d90de6`), so bundles carry Debug IDs -- only the upload step is missing. Recommended fix: add an EAS Build hook script (`eas-hooks/eas-build-on-success.sh` or `pre-upload-artifacts`) that calls `npx sentry-cli sourcemaps upload --org self-employed-s6 --project boges2birds <bundle-dir>`. Helper script already shipped in the SDK at `node_modules/@sentry/react-native/scripts/expo-upload-sourcemaps.js`. `SENTRY_AUTH_TOKEN` is already a sensitive EAS env var on production. EAS hook docs: https://docs.expo.dev/build-reference/build-hooks/. After the hook lands, kick off build 6 and grep the build logs for `Uploading source maps` / `Source map upload completed` to verify.
3. [ ] **TestFlight smoke-check of build 6** -- once source maps are uploading, install build 6 on a device, retrigger the same intentional JS error from step 1, and confirm the Sentry event now shows a readable (un-minified) stack trace pointing back to TypeScript source.
4. [ ] **App Store submission** -- `eas submit --platform ios --profile production` against the build 6 IPA so the first production release ships with working source maps.
